...
void loop()
{
   ...
   { 
      ...
      if (est_pir1 == LOW)  
      {
        ...
        http.addHeader("NODECMU", "2");
        http.addHeader("num", "2");
        http.addHeader("direccion", "a");
        http.addHeader("sen_n", "sen2_a");

        // JUSTO AQUI TAMBIEN MODIFICAR !!!
        int httpCode = http.POST("Sensor 2 A");
...
// Cuando el Pir 2 es activado
   ...
   { 
      ...
      if (est_pir2 == LOW)  
      {
        ...
        http.addHeader("sen_n", "sen2_b");

        // Igual esto a modificar segun el sensor
        int httpCode = http.POST("Sensor 2 B");
        //Serial.print("Request response =  ");
        //Serial.print(httpCode);   //Print HTTP return code
