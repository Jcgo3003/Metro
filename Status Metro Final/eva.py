...
def evaluacion( prom, hora_esp, val):
...
####### Evaluando el tiempo esperado
    ...
    elif( tdel_int >= (val) and tdel_int < (val+8)):
        eva = 2 # Buen servicio

    elif( tdel_int >= (val+8) and tdel_int < (val+16)):
        eva = 3 # Servicio regular

    elif( tdel_int >=(val+16)  and tdel_int < (val+24)):
        eva = 4 # Servicio malo

    elif( tdel_int >=(val+24) and tdel_int < (val+32) ):
